...
function ExerciseModal({ type, sessions, onClose }: { type: ExerciseType; sessions: SessionData[]; onClose: () => void }) {
  ...
  const typeSessions = sessions.filter(s => {
    const typeKey = s.exerciseType && Object.keys(EXERCISE_META).includes(s.exerciseType)
      ? s.exerciseType
      : "practice";
    return typeKey === type;
  });
  const avgAcc = typeSessions.length > 0 ? Math.round(typeSessions.reduce((a, s) => a + s.averageAccuracy, 0) / typeSessions.length) : 0;
  const totalXP = typeSessions.reduce((a, s) => a + (s.xpEarned ?? (s.attempts?.filter((a: any) => a.correct).length * 10)), 0);
...
function ExerciseBreakdown({ sessions, loading, onSelect }: { sessions: SessionData[]; loading: boolean; onSelect: (t: ExerciseType) => void }) {
  ...
  sessions.forEach(s => {
    const typeKey = s.exerciseType && Object.keys(EXERCISE_META).includes(s.exerciseType)
      ? s.exerciseType as ExerciseType
      : "practice";
    if (!counts[typeKey]) counts[typeKey] = { count: 0, totalAcc: 0, totalXP: 0 };
    counts[typeKey]!.count++;
    counts[typeKey]!.totalAcc += s.averageAccuracy;
    counts[typeKey]!.totalXP += s.xpEarned ?? (s.attempts?.filter((a: any) => a.correct).length * 10);
  });
          ...
          <tbody>
            {recent.map((s, i) => {
              const typeKey = s.exerciseType && Object.keys(EXERCISE_META).includes(s.exerciseType)
                ? s.exerciseType
                : "practice";
              const meta = EXERCISE_META[typeKey];
              return (
                <tr key={i} style={{ borderBottom: `1px solid ${COLORS.borderLight}` }}>
                  ...
                  <td style={{ padding: "16px 12px", color: COLORS.lavender }}>{s.wordsCompleted ?? s.attempts?.length ?? 0} / {s.totalWords ?? s.attempts?.length ?? 0}</td>
                  <td style={{ padding: "16px 12px" }}>
                  ...
                  <td style={{ padding: "16px 12px", color: COLORS.xp, fontWeight: 700, fontSize: "1rem" }}>+{s.xpEarned ?? (s.attempts?.filter((a: any) => a.correct).length * 10)}</td>
